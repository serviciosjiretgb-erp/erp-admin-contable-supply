...
import { 
  ...
  Database, Trash2, ArrowUpRight, ArrowDownRight, BookOpen, Filter, Users, Briefcase
} from 'lucide-react';
...
function EstadoResultadoView({ onBack, dbData }) {
  ...
  const tree = useMemo(() => {
    ...
    resData.forEach(item => {
      ...
      const isVenta = item.name.toUpperCase().includes('VENTA') || item.name.toUpperCase().includes('INGRESO') || item.path.toUpperCase().includes('VENTA');
      if (isVenta) {
      ...
      if (!leaf) cur.push({ n: itemName.trim(), u: item.usd, b: item.bs, isLeaf: true });
    ...
    const sortTree = (nodes, level) => {
      nodes.sort((a,b) => {
        if (level === 0) {
          const order = { "INGRESO": 1, "VENTA": 1, "COSTO": 2, "GASTO": 3, "EGRESO": 3 };
          const aUpper = a.n.toUpperCase(); const bUpper = b.n.toUpperCase();
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white border-b-2 border-orange-500 p-4 flex justify-between items-center sticky top-0 z-30 shadow-md">
        <button onClick={onBack} className="flex items-center gap-2 font-black text-xs text-black uppercase hover:text-orange-600 transition-colors"><ArrowLeft size={16}/> Volver</button>
      ...
      <main className="p-8 max-w-6xl mx-auto pb-16">
        <div className="bg-white px-8 py-10 border-t-8 border-black shadow-xl flex flex-col items-center text-center mb-6 rounded-b-2xl">
          ...
          <div className="w-24 h-1.5 bg-orange-500 mb-4 rounded-full"/>
          <h2 className="text-xl font-black text-gray-800 uppercase tracking-widest border-b border-gray-100 pb-2 mb-4 w-full max-w-md">Estado de Resultado</h2>
...
function AnalisisComparativoView({ onBack, dbData }) {
  ...
  const tree = useMemo(() => {
    ...
    const processItem = (item, isM1) => {
      ...
      let accName = item.name;
      if (item.name.toUpperCase().includes('VENTA') || item.name.toUpperCase().includes('INGRESO') || item.path.toUpperCase().includes('VENTA')) {
        accName = "4.1.01.001 - INGRESOS GENERALES POR VENTA";
      }
      let categoryNode = root.find(n => n.key === mainCategory);
      if (!categoryNode) { categoryNode = { key: mainCategory, n: pathParts[0]?.trim().toUpperCase() || 'OTROS', c: [], m1_u: 0, m2_u: 0 }; root.push(categoryNode); }
    ...
    m1Data.forEach(item => processItem(item, true)); m2Data.forEach(item => processItem(item, false));
    ...
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white border-b-2 border-black p-4 flex justify-between items-center sticky top-0 z-30 shadow-md">
        ...
        <div className="flex gap-2">
          <select value={month1} onChange={(e) => setMonth1(e.target.value)} className="text-xs p-1 border rounded font-bold uppercase">{availableMonths.map(m => <option key={m} value={m}>{m}</option>)}</select>
          <span className="font-bold text-xs pt-1">VS</span>
          <select value={month2} onChange={(e) => setMonth2(e.target.value)} className="text-xs p-1 border rounded font-bold uppercase bg-orange-50">{availableMonths.map(m => <option key={m} value={m}>{m}</option>)}</select>
        </div>
      </header>
      <main className="p-8 max-w-6xl mx-auto">
        <div className="bg-white p-10 border-t-8 border-black shadow-xl text-center mb-6 rounded-b-2xl">
          <h1 className="text-3xl font-black mb-2 tracking-tighter">ANÁLISIS DE VARIACIONES</h1>
          <div className="w-20 h-1.5 bg-orange-500 mx-auto rounded-full mb-4"/>
          <p className="bg-black text-white px-5 py-2 inline-block rounded-full font-black text-[10px] uppercase">{month1} VS {month2}</p>
        </div>
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden border">
          <table className="w-full text-left">
            <thead className="bg-black text-white text-[10px] uppercase font-black">
              <tr><th className="p-5">Estructura</th><th className="p-5 text-right">{month1}</th><th className="p-5 text-right">{month2}</th><th className="p-5 text-right">Var. Absoluta</th><th className="p-5 text-right">Var. %</th></tr>
            </thead>
            <tbody>
              {tree.map((cat, i) => {
                const isIngreso = cat.n.includes('INGRESO') || cat.n.includes('VENTA') || cat.key.startsWith('4');
                return (
                  <React.Fragment key={i}>
                    <tr className="bg-gray-200 border-b border-gray-300"><td colSpan={5} className="p-3 font-black text-xs uppercase">{cat.n}</td></tr>
                    {cat.c.map((acc, j) => {
                      const vAbs = acc.m2_u - acc.m1_u; const vPct = acc.m1_u !== 0 ? (vAbs / Math.abs(acc.m1_u)) * 100 : (acc.m2_u !== 0 ? 100 : 0);
                      const fav = isIngreso ? vAbs > 0 : vAbs < 0;
                      return (
                        <tr key={j} className="border-b hover:bg-orange-50">
                          <td className="p-3 pl-8 text-[11px] font-bold uppercase">{acc.n}</td>
                          <td className="p-3 text-right font-mono">{fmtR(acc.m1_u)}</td>
                          <td className="p-3 text-right font-mono font-bold">{fmtR(acc.m2_u)}</td>
                          <td className={`p-3 text-right font-mono font-bold ${vAbs !== 0 ? 'text-orange-600' : ''}`}>{fmtR(vAbs)}</td>
                          <td className={`p-3 text-right font-mono font-black flex items-center justify-end gap-1 ${fav ? 'text-green-600' : 'text-red-500'}`}>
                            {vAbs > 0 ? <ArrowUpRight size={14}/> : <ArrowDownRight size={14}/>} {Math.abs(vPct).toFixed(2)}%
                          </td>
                        </tr>
                      );
                    })}
              ...
              <tr className="bg-black text-white font-black">
                <td className="p-6 uppercase">RESULTADO NETO</td>
                <td className="p-6 text-right">{fmtR(total_m1)}</td><td className="p-6 text-right">{fmtR(total_m2)}</td>
                <td className="p-6 text-right text-orange-400">{fmtR(varAbsTotal)}</td>
                <td className={`p-6 text-right flex items-center justify-end gap-2 ${varAbsTotal > 0 ? 'text-green-400' : 'text-red-400'}`}>
...
function BalanceGeneralView({ onBack, dbData }) {
  ...
  let totalActivos = 0; tree.forEach(n => { if(n.n.toUpperCase().includes('ACTIVO')) totalActivos += n.u; });
  const fmtR = (v) => new Intl.NumberFormat('es-VE', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(Math.abs(v));

  return (
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white border-b-2 border-black p-4 flex justify-between items-center sticky top-0 z-30">
        <button onClick={onBack} className="flex items-center gap-2 font-black text-xs uppercase hover:text-orange-600"><ArrowLeft size={16}/> Volver</button>
        <div className="flex gap-4">
          <select value={selectedMonth} onChange={(e) => setSelectedMonth(e.target.value)} className="text-xs p-1 border rounded font-bold">{availableMonths.map(m => <option key={m} value={m}>{m}</option>)}</select>
          <input type="number" value={tasa} onChange={e => setTasa(parseFloat(e.target.value) || 1)} className="text-xs border p-1 rounded w-20 font-bold" />
        </div>
      </header>
      <main className="p-8 max-w-6xl mx-auto">
        <div className="bg-white p-10 border-t-8 border-black text-center mb-6 rounded-b-2xl shadow-xl">
          <h1 className="text-3xl font-black uppercase">Balance General</h1>
          <div className="w-24 h-1.5 bg-orange-500 mx-auto mt-4 rounded-full" />
        </div>
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden border">
          <table className="w-full text-left">
            <thead className="bg-black text-white text-[10px] uppercase font-black">
              <tr><th className="p-5">Estructura</th><th className="p-5 text-right">USD</th><th className="p-5 text-right">Equiv. Bs.</th><th className="p-5 text-right">%</th></tr>
            </thead>
            <tbody>
    ...
    <div className="min-h-screen bg-white">
      <header className="bg-black text-white p-4 flex justify-between items-center sticky top-0 z-30 shadow-lg border-b-4 border-orange-500">
        ...
        <div className="flex gap-4 items-center">
          <select value={mesFiltro} onChange={e => setMesFiltro(e.target.value)} className="bg-gray-900 text-white text-[10px] rounded p-1 font-bold">
          ...
          <input type="date" value={fechaCorte} onChange={e => setFechaCorte(e.target.value)} className="bg-gray-900 text-white text-[10px] rounded p-1 font-bold" />
        </div>
      </header>
      <main className="p-8 max-w-[1200px] mx-auto pb-24">
        <div className="mb-10 text-center">
          <h1 className="text-3xl font-black text-black uppercase tracking-tighter">Auxiliar de Activos Fijos</h1>
          <div className="w-24 h-1.5 bg-orange-500 mx-auto mt-4 rounded-full" />
        </div>
          ...
          {Object.keys(categories).sort().map(catName => (
            <div key={catName} className="bg-white">
              <div className="flex justify-between items-end border-b-4 border-black pb-2 mb-4">
                <h2 className="text-2xl font-black uppercase tracking-tight">{catName}</h2>
                <p className="text-xl font-mono font-black">USD {fmtCur(categories[catName].totalNeto)}</p>
              </div>
              <table className="w-full text-left text-[11px] border-collapse">
                <thead className="bg-gray-100 font-black uppercase text-gray-600">
                  <tr><th className="p-3">Código</th><th className="p-3">Descripción</th><th className="p-3 text-center">Fecha</th><th className="p-3">Proveedor</th><th className="p-3 text-right">Costo USD</th><th className="p-3 text-right">Dep. Acum.</th><th className="p-3 text-right">Valor Neto</th></tr>
                </thead>
                <tbody>
                  {categories[catName].items.map((a, i) => (
                    <tr key={i} className="border-b border-gray-100 hover:bg-orange-50">
                      <td className="p-3 font-mono text-gray-400">{a.codigo}</td>
                      <td className="p-3 font-black">{a.descripcion} {a.esMejora && <span className="ml-2 bg-black text-white text-[8px] px-1 rounded">MEJORA</span>}</td>
                      <td className="p-3 text-center font-mono">{a.fechaAdquisicion.split('-').reverse().join('/')}</td>
                      <td className="p-3 text-gray-500 font-bold">{a.proveedor}</td>
                      <td className="p-3 text-right font-mono font-black">{fmtCur(a.valorAdquisicion)}</td>
        ...
        <div className="mt-16 bg-black p-8 rounded-2xl flex justify-between items-center shadow-2xl border-l-8 border-orange-500">
          <p className="text-white font-black text-3xl uppercase">Total Activos Fijos Netos</p>
          <p className="text-white text-4xl font-mono font-black">USD {fmtR(granTotalNeto)}</p>
        </div>
      </main>
    </div>
  );
...
// ============================================================================
// 7. COMPONENTE PRINCIPAL / DASHBOARD
// ============================================================================
export default function ReportesFinancierosApp() {
  const [view, setView] = useState('dashboard');
  const [dbData, setDbData] = useState(() => JSON.parse(localStorage.getItem('jiret_db') || '[]'));
  const [activosData, setActivosData] = useState(() => JSON.parse(localStorage.getItem('jiret_activos') || '[]'));
  
  useEffect(() => { localStorage.setItem('jiret_db', JSON.stringify(dbData)); }, [dbData]);
  useEffect(() => { localStorage.setItem('jiret_activos', JSON.stringify(activosData)); }, [activosData]);
  ...
  return (
    <div className="min-h-screen bg-white">
      <header className="px-8 py-5 bg-black border-b-4 border-orange-500 flex justify-between items-center shadow-xl">
        <h1 className="text-white font-black text-2xl tracking-widest uppercase">JIRET G&B <span className="text-orange-500">Finance</span></h1>
        <button onClick={() => setView('config')} className="bg-white px-4 py-2 rounded font-black text-[10px] uppercase">Configuración</button>
      </header>
      <main className="max-w-6xl mx-auto px-6 py-20 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        <button onClick={() => setView('resultado')} className="bg-white border-2 border-orange-500 p-10 rounded-2xl text-left hover:shadow-2xl transition-all group">
          ...
          <p className="text-gray-500 text-xs font-bold mt-2">INGRESOS {'>'} COSTOS {'>'} GASTOS</p>
        </button>
        <button onClick={() => setView('balance')} className="bg-black p-10 rounded-2xl text-left hover:shadow-2xl transition-all group">
          <Scale className="text-orange-500 mb-4 group-hover:scale-110 transition-transform" size={48} />
          <h3 className="font-black text-2xl text-white uppercase">Balance</h3>
          <p className="text-gray-400 text-xs font-bold mt-2">SITUACIÓN FINANCIERA</p>
        </button>
        <button onClick={() => setView('comparativo')} className="bg-white border-2 border-orange-500 p-10 rounded-2xl text-left hover:shadow-2xl transition-all group">
          <GitCompare className="text-orange-500 mb-4 group-hover:scale-110 transition-transform" size={48} />
          <h3 className="font-black text-2xl uppercase">Variaciones</h3>
          <p className="text-gray-500 text-xs font-bold mt-2">MES VS MES</p>
        </button>
        <button onClick={() => setView('inversiones')} className="bg-black p-10 rounded-2xl text-left hover:shadow-2xl transition-all group">
          <Landmark className="text-orange-500 mb-4 group-hover:scale-110 transition-transform" size={48} />
          <h3 className="font-black text-2xl text-white uppercase">Inversiones</h3>
          <p className="text-gray-400 text-xs font-bold mt-2">ACTIVOS FIJOS Y MEJORAS</p>
        </button>
        <button className="bg-gray-100 p-10 rounded-2xl text-left opacity-50 cursor-not-allowed">
          <Users className="text-gray-400 mb-4" size={48} />
          <h3 className="font-black text-2xl uppercase">Aux. CxC</h3>
          <p className="text-gray-400 text-xs font-bold mt-2">EN CONSTRUCCIÓN</p>
        </button>
        <button className="bg-gray-100 p-10 rounded-2xl text-left opacity-50 cursor-not-allowed">
          <Briefcase className="text-gray-400 mb-4" size={48} />
          <h3 className="font-black text-2xl uppercase">Aux. CxP</h3>
          <p className="text-gray-400 text-xs font-bold mt-2">EN CONSTRUCCIÓN</p>
        </button>
      </main>
      ...
      {view === 'config' && (
        <div className="fixed inset-0 bg-black/90 flex items-center justify-center p-6 z-50">
          <div className="bg-white p-10 rounded-3xl w-full max-w-lg shadow-2xl border-t-8 border-orange-500">
            <h2 className="text-3xl font-black uppercase mb-6">Panel de Carga</h2>
            <div className="space-y-4">
              <label className="block p-4 bg-gray-100 rounded-xl cursor-pointer hover:bg-orange-100">
                <p className="font-black text-[10px] uppercase text-gray-500">Estados de Resultado / Balance</p>
                <input type="file" multiple className="mt-2" onChange={async (e) => {
                  const data = await processFiles(e.target.files);
                  setDbData(prev => [...prev, ...data]);
                  alert("✅ Datos contables cargados");
                }} />
              </label>
              <label className="block p-4 bg-gray-100 rounded-xl cursor-pointer hover:bg-orange-100">
                <p className="font-black text-[10px] uppercase text-gray-500">Auxiliar Activos Fijos</p>
                <input type="file" className="mt-2" onChange={async (e) => {
                  const data = await processActivosFijos(e.target.files[0]);
                  setActivosData(data);
                  alert("✅ Activos fijos cargados");
                }} />
              </label>
            </div>
            <div className="mt-6 flex justify-between">
               <button onClick={() => { if(window.confirm("¿Limpiar todo?")) {setDbData([]); setActivosData([]); }}} className="text-red-500 font-black text-[10px] uppercase underline">Borrar Datos</button>
               <button onClick={() => setView('dashboard')} className="bg-black text-white px-8 py-3 rounded-xl font-black uppercase text-xs">Cerrar</button>
            </div>
          </div>
        </div>
      )}
